...
3.  Click "Load segment data" and select the corresponding `classifications.json` file
    generated by the pipeline (located in `data/2_final/your_video_name/`).

The interface will display the video and a list of segment cards. As the video plays, the corresponding segment card will be highlighted. You can also click the timestamps on any card to navigate the video to that segment's start/end time.

**Note:** The tool works entirely offline. You can open the HTML file directly from your file system (`file://`).
