    ...
    def _call_hf_processor(
        ...
        if videos := mm_data.pop("videos", []):
            ...
            for item_idx, item in enumerate(videos):
                ...
                # [VIDEO DEBUG] diagnostic patch: log the true post-smart_resize
                # dimensions of the frames that will be embedded and passed to
                # the vision encoder. The decoded shape logged by
                # VideoBackend.load_bytes is pre-resize; these are the values
                # actually embedded.
                grid_t, grid_h, grid_w = video_grid_thw[0].tolist()
                patch_size = hf_config.vision_config.patch_size
                temporal_patch_size = hf_config.vision_config.temporal_patch_size
                logger.warning(
                    "[VIDEO DEBUG] Qwen3VLMultiModalProcessor._call_hf_processor "
                    "embedded frames:\n"
                    "  Decoded input : frames shape=%s (N,H,W,C), backend=%s, "
                    "duration=%.2f s\n"
                    "  Size budget   : %s (HF smart_resize input, from "
                    "mm_processor_kwargs)\n"
                    "  Patch grid    : t=%d h=%d w=%d (patch_size=%d, "
                    "merge_size=%d, temporal_patch_size=%d)\n"
                    "  Embedded      : %d frames (incl. temporal padding), "
                    "%d x %d px per frame\n"
                    "  Tokens        : %d per frame, %d total for this video "
                    "(before pruning)",
                    tuple(getattr(video_array, "shape", ()) or ()),
                    getattr(metadata, "video_backend", None),
                    float(getattr(metadata, "duration", 0.0) or 0.0),
                    video_mm_kwargs.get("size"),
                    grid_t,
                    grid_h,
                    grid_w,
                    patch_size,
