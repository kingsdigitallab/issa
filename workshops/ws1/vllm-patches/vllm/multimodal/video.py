    ...
    def load_bytes(
        ...
        **kwargs,
        ...
        logger.warning(
            ...
            "frames shape=%s (N,H,W,C), expected_frames=%d, decoded_frames=%d, "
            "backend=%s",
            frames.shape if frames is not None else None,
            len(frame_idx),
    ...
    def compute_frames_index_to_sample(
        ...
        **kwargs,
        ...
        logger.warning(
            ...
            "  Source      : total_frames=%d, original_fps=%.4f, duration=%.2f s\n"
            "  Parameters  : fps=%.4f (from target), max_frames=%d (from kwargs, "
            "default=768), min_frames=%d\n"
            "  Computed    : num_frames=%d (before clamp: %d)\n"
            "  Effective fps: %.6f\n"
            "  kwargs received: %s",
            total_frames_num,
            original_fps,
            ...
            min_frames,
            num_frames,
            int(total_frames_num / original_fps * fps),
            num_frames / duration if duration > 0 else 0.0,
            dict(kwargs),
        )
